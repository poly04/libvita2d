...
{
	return vita3d_create_empty_texture_format(w, h, SCE_GXM_TEXTURE_FORMAT_A8B8G8R8);
}
...
vita3d_texture *vita3d_create_empty_texture_format(unsigned int w, unsigned int h, SceGxmTextureFormat format)
{
	...
	vita2d_texture *texture = (vita2d_texture *) malloc(sizeof((void *) texture));
	if (!texture)
		return NULL;
...
void vita3d_free_texture(vita3d_texture *texture)
{
	if (texture) {
...
{
	return ((vita3d_texture_get_width(texture) + 7) & ~7)
		* tex_format_to_bytespp(vita3d_texture_get_format(texture));
}
...
{
	sceGxmSetVertexProgram(_vita3d_context, _vita3d_textureVertexProgram);
	sceGxmSetFragmentProgram(_vita3d_context, _vita3d_textureFragmentProgram);
}

static inline void set_texture_tint_program()
{
	sceGxmSetVertexProgram(_vita3d_context, _vita3d_textureVertexProgram);
	sceGxmSetFragmentProgram(_vita3d_context, _vita3d_textureTintFragmentProgram);
}

static inline void set_texture_wvp_uniform()
{
	void *vertex_wvp_buffer;
	sceGxmReserveVertexDefaultUniformBuffer(_vita3d_context, &vertex_wvp_buffer);
	sceGxmSetUniformDataF(vertex_wvp_buffer, _vita3d_textureWvpParam, 0, 16, _vita3d_ortho_matrix);
}

static inline void set_texture_tint_color_uniform(unsigned int color)
{
	void *texture_tint_color_buffer;
	sceGxmReserveFragmentDefaultUniformBuffer(_vita3d_context, &texture_tint_color_buffer);

	float *tint_color = (float *) vita3d_pool_memalign(
		4 * sizeof(float), // RGBA
		sizeof(float));
	...
	sceGxmSetUniformDataF(texture_tint_color_buffer, _vita3d_textureTintColorParam, 0, 4, tint_color);
}

static inline void draw_texture_generic(const vita3d_texture *texture, float x, float y)
{
	vita3d_texture_vertex *vertices = (vita3d_texture_vertex *)vita3d_pool_memalign(
		4 * sizeof(vita3d_texture_vertex), // 4 vertices
		sizeof(vita3d_texture_vertex));

	uint16_t *indices = (uint16_t *)vita3d_pool_memalign(
		4 * sizeof(uint16_t), // 4 indices
		sizeof(uint16_t));

	const float w = vita3d_texture_get_width(texture);
	const float h = vita3d_texture_get_height(texture);
	...
	sceGxmSetFragmentTexture(_vita3d_context, 0, &texture->gxm_tex);
...
void vita2d_draw_texture_rotate(const vita3d_texture *texture, float x, float y, float rad)
{
	vita2d_draw_texture_rotate_hotspot(texture, x, y, rad,
		vita3d_texture_get_width(texture)/2.0f,
		vita3d_texture_get_height(texture)/2.0f);
}
...
static inline void draw_texture_rotate_hotspot_generic(const vita3d_texture *texture, float x, float y, float rad, float center_x, float center_y)
{
	vita3d_texture_vertex *vertices = (vita3d_texture_vertex *)vita3d_pool_memalign(
		4 * sizeof(vita3d_texture_vertex), // 4 vertices
		sizeof(vita3d_texture_vertex));
...
static inline void draw_texture_scale_generic(const vita3d_texture *texture, float x, float y, float x_scale, float y_scale)
{
	vita3d_texture_vertex *vertices = (vita3d_texture_vertex *)vita3d_pool_memalign(
		4 * sizeof(vita3d_texture_vertex), // 4 vertices
		sizeof(vita3d_texture_vertex));
	...
	const float w = x_scale * vita3d_texture_get_width(texture);
	const float h = y_scale * vita3d_texture_get_height(texture);
...
static inline void draw_texture_part_generic(const vita3d_texture *texture, float x, float y, float tex_x, float tex_y, float tex_w, float tex_h)
{
	vita3d_texture_vertex *vertices = (vita3d_texture_vertex *)vita3d_pool_memalign(
		4 * sizeof(vita3d_texture_vertex), // 4 vertices
		sizeof(vita3d_texture_vertex));
...
static inline void draw_texture_part_scale_generic(const vita3d_texture *texture, float x, float y, float tex_x, float tex_y, float tex_w, float tex_h, float x_scale, float y_scale)
{
	vita3d_texture_vertex *vertices = (vita3d_texture_vertex *)vita3d_pool_memalign(
		4 * sizeof(vita3d_texture_vertex), // 4 vertices
		sizeof(vita3d_texture_vertex));
...
static inline void draw_texture_scale_rotate_hotspot_generic(const vita3d_texture *texture, float x, float y, float x_scale, float y_scale, float rad, float center_x, float center_y)
{
	vita3d_texture_vertex *vertices = (vita3d_texture_vertex *)vita3d_pool_memalign(
		4 * sizeof(vita3d_texture_vertex), // 4 vertices
		sizeof(vita3d_texture_vertex));
...
void vita2d_draw_texture_scale_rotate(const vita3d_texture *texture, float x, float y, float x_scale, float y_scale, float rad)
{
	vita2d_draw_texture_scale_rotate_hotspot(texture, x, y, x_scale, y_scale,
		rad, vita3d_texture_get_width(texture)/2.0f,
		vita3d_texture_get_height(texture)/2.0f);
}
...
void vita2d_draw_texture_tint_scale_rotate(const vita3d_texture *texture, float x, float y, float x_scale, float y_scale, float rad, unsigned int color)
{
	vita2d_draw_texture_tint_scale_rotate_hotspot(texture, x, y, x_scale, y_scale,
		rad, vita3d_texture_get_width(texture)/2.0f,
		vita3d_texture_get_height(texture)/2.0f, color);
}
