...
vita2d_pgf *vita2d_load_default_pgf()
{
	...
	vita2d_pgf *font = (vita2d_pgf *)malloc(sizeof(*font));
	if (!font)
		return NULL;
...
static int atlas_add_glyph(vita2d_pgf *font, unsigned int character)
{
	...
	bp2d_size size = {
		(int)char_info.bitmapWidth,
		(int)char_info.bitmapHeight
	};

	texture_atlas_entry_data data = {
		(int)char_info.bitmapLeft,
		(int)char_info.bitmapTop,
	...
	texture_data = vita3d_texture_get_datap(tex);
	...
	glyph_image.bufWidth = vita3d_texture_get_width(tex);
	glyph_image.bufHeight = vita3d_texture_get_height(tex);
	glyph_image.bytesPerLine = vita3d_texture_get_stride(tex);
	glyph_image.pad = 0;
