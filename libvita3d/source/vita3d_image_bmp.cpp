	...
	void (*read_fn)(void *user_data, void *buffer, unsigned int length))
{
	...
	vita3d_texture *texture = vita3d_create_empty_texture(
		bmp_ih->biWidth,
		bmp_ih->biHeight);
	...
	unsigned int tex_stride = vita3d_texture_get_stride(texture);
	...
	for (i = 0; i < bmp_ih->biHeight; i++) {
		...
		unsigned int *tex_ptr = (unsigned int *)(((uint32_t)texture_data) + y*tex_stride);

		for (x = 0; x < bmp_ih->biWidth; x++) {

			if (bmp_ih->biBitCount == 32) {		//ABGR8888
				unsigned int color = *(unsigned int *)(((uint32_t)buffer) + x*4);
				*tex_ptr = (color&0xFF)<<24 | ((color>>8)&0xFF)<<16 |
					((color>>16)&0xFF)<<8 | (color>>24);

			} else if (bmp_ih->biBitCount == 24) {	//BGR888
				unsigned char *address = (unsigned char *) ((uint32_t)buffer) + x*3;
				*tex_ptr = (*address)<<16 | (*(address+1))<<8 |
					(*(address+2)) | (0xFF<<24);

			} else if (bmp_ih->biBitCount == 16) {	//BGR565
				unsigned int color = *(unsigned short *)(((uint32_t)buffer) + x*2);
...
vita2d_texture *vita2d_load_BMP_buffer(const void *buffer)
{
	...
	BITMAPINFOHEADER bmp_ih;
	memcpy(&bmp_ih, (const void *) ((uint32_t)buffer) + sizeof(BITMAPFILEHEADER), sizeof(BITMAPINFOHEADER));
