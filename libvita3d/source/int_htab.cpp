...
{
	int_htab *htab = (int_htab *)malloc(sizeof(*htab));
	if (!htab)
		return NULL;
	...
	htab->entries = (int_htab_entry *)malloc(htab->size * sizeof(*htab->entries));
...
void int_htab_resize(int_htab *htab, unsigned int new_size)
{
	...
	htab->size = new_size;
	htab->used = 0;
	htab->entries = (int_htab_entry *)malloc(new_size * sizeof(*htab->entries));
