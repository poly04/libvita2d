...
{
	texture_atlas *atlas = (texture_atlas *)malloc(sizeof(*atlas));
	if (!atlas)
		return NULL;
	...
	atlas->texture = vita3d_create_empty_texture_format(width,
							    height,
							    format);
	if (!atlas->texture) {
		free(atlas);
	...
	vita3d_texture_set_filters(atlas->texture,
...
{
	vita3d_free_texture(atlas->texture);
			 ...
			 bp2d_position *inserted_pos)
{
	...
	entry = (atlas_htab_entry *)malloc(sizeof(*entry));
...
{
	atlas_htab_entry *entry = (atlas_htab_entry *)int_htab_find(atlas->htab, character);
	if (!entry)
		return 0;
