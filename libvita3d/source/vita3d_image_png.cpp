...
static void _vita3d_read_png_file_fn(png_structp png_ptr, png_bytep data, png_size_t length)
{
	SceUID fd = *(SceUID*) png_get_io_ptr(png_ptr);
	sceIoRead(fd, data, length);
}

static void _vita3d_read_png_buffer_fn(png_structp png_ptr, png_bytep data, png_size_t length)
{
...
static vita3d_texture *_vita3d_load_PNG_generic(const void *io_ptr, png_rw_ptr read_data_fn)
{
	...
	vita3d_texture *texture = vita3d_create_empty_texture(width, height);
	if (!texture)
	{
	...
	unsigned int stride = vita3d_texture_get_stride(texture);

	unsigned int i;
	for (i = 0; i < height; i++) {
		row_ptrs[i] = (png_bytep)(((uint32_t) texture_data) + i*stride);
	}
...
vita3d_texture *vita3d_load_PNG_file(const char *filename)
{
	...
	vita3d_texture *texture = _vita3d_load_PNG_generic((void *)&fd, _vita3d_read_png_file_fn);
	sceIoClose(fd);
	return texture;
...
vita3d_texture *vita3d_load_PNG_buffer(const void *buffer)
{
	...
	return _vita3d_load_PNG_generic((void *)&buffer_address, _vita3d_read_png_buffer_fn);
}
