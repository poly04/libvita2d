...
vita2d_font *vita2d_load_font_file(const char *filename)
{
	...
	vita2d_font *font = (vita2d_font *)malloc(sizeof(*font));
	if (!font)
		return NULL;
			   ...
			   const FT_BitmapGlyph bitmap_glyph, int glyph_size)
{
	...
	texture_data = vita3d_texture_get_datap(atlas->texture);
	tex_width = vita3d_texture_get_width(atlas->texture);

	for (i = 0; ((int)i) < size.h; i++) {
